...
class KeyRouter:
    ...
    def _on_key(self, _ctl, keyval, _keycode, state) -> bool:
        ...
        catcher = getattr(self._window, "wake_catcher", None)
        if catcher is not None and catcher.get_visible():
            catcher.on_wake()            # US-30: the screen is asleep; the key only wakes it
            return True
